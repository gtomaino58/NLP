# Version VGG19 con 2 capas, Dropout y Normalizacion, Adam, BS = 8, LR 10-5 y 100 Epochs
...
if __name__ == '__main__':
    ...
    # Vamos a guardar el modelo entrenado
    torch.save(model.state_dict(), path_res + '/vgg19_skin_cancer_model_F.pth')
    ...
    plt.title('Train and Test Loss vs Epochs')
    plt.grid()
    plt.savefig(path_res + '/train_test_loss_vs_epochs_VGG19_F.png')
    ...
    plt.title('Train and Test Accuracy vs Epochs')
    plt.grid()
    plt.savefig(path_res + '/train_test_accuracy_vs_epochs_VGG19_F.png')
    plt.close()  # Close the plot to free up memory
    ...
    plt.xlabel('Predicted Label')
    plt.ylabel('True Label')
    plt.savefig(path_res + '/confusion_matrix_VGG19_F.png')
    plt.close()  # Close the plot to free up memory
    ...
    with open(path_res + '/classification_report_VGG19_F.txt', 'w') as f:
        f.write(report_df.to_string())

    # Save the classification report to a CSV file
    report_df.to_csv(path_res + '/classification_report_VGG19_F.csv', index=True)

    # Save the classification report to a JSON file
    report_df.to_json(path_res + '/classification_report_VGG19_F.json', orient='index')